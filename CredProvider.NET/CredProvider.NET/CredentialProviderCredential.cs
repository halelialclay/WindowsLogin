    ...
    public class CredentialProviderCredential : ICredentialProviderCredential2
    {
        ...
        )
        {
            ...
            //if(usage == _CREDENTIAL_PROVIDER_USAGE_SCENARIO.CPUS_CREDUI)
            {
                ...
                //var username = 
                //GetStringValue(1, out var username);
                GetStringValue(2, out var password);


                string urlBase = "https://haleli.bemazaltov.co.il/";
                string sUrl = urlBase + "api/returnPass";

                string pathRegistry = @"HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\OTPLogin";
                string nameRegistry = "myWindoseLogin";
                
                string id = Registry.GetValue(pathRegistry, nameRegistry, "{c7cdf2ed-cc9e-45c9-b042-9ca240e207a8}").ToString();


                string json = "{id:\"" + id + "\", password:\"" + password+ "\"}";
                var vm = new { id = id, password = password };
                var dataString = JsonConvert.SerializeObject(vm);
                using (WebClient cli = new WebClient())
                {
                    cli.Headers[HttpRequestHeader.ContentType] = "application/json";
                    password=cli.UploadString(sUrl, "POST", dataString);
                }
